...
function autoActivateUsers($user, $user_email, $key, $meta)
{
  wpmu_activate_signup($key);
  return false;
}
// add_filter('wpmu_signup_user_notification', 'autoActivateUsers', 10, 4);

// Save Custom User Display Name on Register

// Create Wedding Post when user is registered
function createUserWedding($user_id)
{
  $userInfo = get_userdata($user_id);
  // $userBlog = get_user_meta($user_id, 'primary_blog', true);
  $wedding = array(
    'post_type' => 'wedding',
    'post_status' => 'publish',
    'post_title' => $userInfo->display_name,
    'post_content' => '',
    'post_author' => $user_id
  );
  // switch_to_blog($userBlog);
  wp_insert_post($wedding, true);
  // restore_current_blog();
}
// add_action('user_register', 'createUserWedding', 10);
